...
double normalizeDegrees(double degrees) {
    double newDegrees = glm::mod(degrees, 360.0);
    if (newDegrees < 0.0) {
        newDegrees += 360.0;
    }
    return newDegrees;
...
double normalizeRadians(double rads) {
    ...
    double normal = glm::mod(rads, TWO_PI);
    normal += (normal < 0) * TWO_PI;
    return normal;
}
...
double rad2deg(double rads) {
    double deg = rads * (180.0 / glm::pi<double>());
    return deg;
}
...
double deg2rad(double degs) {
    double rad = degs * (glm::pi<double>() / 180);
    return rad;
}
...
double areaOfTriangle(double x1, double y1, double x2, double y2, double x3, double y3) {
    double sideOne = distanceBetween(x1, y1, x2, y2);
    double sideTwo = distanceBetween(x2, y2, x3, y3);
    double sideThree = distanceBetween(x3, y3, x1, y1);
    
    double area = areaOfTriangle(sideOne, sideTwo, sideThree);
    return area;
}
...
dvec2 pointOnCircle(const dvec2& center, double R, double angleRads) {
    double x = R * cos(angleRads) + center.x;
    double y = R * sin(angleRads) + center.y;
    x = glm::round(x * 1e6) / 1e6;  // rounds to 6 decimals
    y = glm::round(y * 1e6) / 1e6;
	return dvec2(x, y);;
}
...
void mouseUtility(GLFWwindow* window, int button, int action, int modes) {
#ifndef CONSOLE_ONLY
...
#ifdef QUARTER_DISPLAY
		xDebug *= 2;
		yDebug *= 2;
#endif

		cout << "Clicked position: (" << xDebug << "," << yDebug << ") = " << endl;
	}
#endif
}

void keyboardUtility(GLFWwindow* window, int key, int scancode, int action, int mods) {
#ifndef CONSOLE_ONLY
	if (action != GLFW_PRESS)
		return;
...
void initGraphics(int W, int H, const char* title,
	...
	string shortTitle = extractBaseFilename(title);
	...
	window = glfwCreateWindow(W, H, shortTitle.c_str(), NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return;
